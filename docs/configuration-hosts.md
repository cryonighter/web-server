...
<config name="app-with-api">
    ...
    <path name="php" regexp="\.php(\?|$)">
        ...
        <handler type="cgi" executable="php-cgi" />
    </path>
</config>
```
...
### `type="cgi"` — обработка запроса CGI-скриптом

Обрабатывает запрос с помощью CGI-скрипта.
...
|--------------|--------------|--------------------------------------------------------|
| `name`       | Нет          | Уникальное имя обработчика (для логирования и отладки) |
| `executable` | Да           | Команда или путь к исполняемому файлу cgi-обработчика  |
